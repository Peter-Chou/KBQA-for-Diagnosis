...
class MedicalExtractor(object):
  ...
  def __init__(self):
    ...
    # self.graph = Graph(host=HOST, http_port=PORT, user=USER, password=PASSWORD)
    self.graph = Graph(f"bolt://{HOST}:{PORT}", auth=(USER, PASSWORD))
      ...
      for line in tqdm(f.readlines(), ncols=80):
        ...
        if 'cure_department' in data_json:
          ...
          if len(cure_department) == 1:
            self.rels_category.append(
                [disease, 'cure_department', cure_department[0]])
        ...
        if 'recommand_eat' in data_json:
          ...
          for _recommand in recommand_eat:
            self.rels_recommandeat.append(
                [disease, 'recommand_recipes', _recommand])
  ...
  def export_entitys_relations(self):
    ...
    self.export_data(self.rels_recommandeat,
                     './graph_data/rels_recommandeat.json')
    self.export_data(self.rels_commonddrug,
                     './graph_data/rels_commonddrug.json')
    self.export_data(self.rels_recommanddrug,
                     './graph_data/rels_recommanddrug.json')
    self.export_data(self.rels_check, './graph_data/rels_check.json')
    self.export_data(self.rels_drug_producer,
                     './graph_data/rels_drug_producer.json')
